...
import ma.WhiteLab.repository.modules.patient.api.DossierMedicalRepository;
...
public class DossierMedicalRepositoryImpl implements DossierMedicalRepository {
    ...
    public List<DossierMedical> findAll() {
        String sql = "SELECT * FROM DossiersMedical ORDER BY id DESC";
    ...
    public DossierMedical findById(Long id) {
        String sql = "SELECT * FROM DossiersMedical WHERE id = ?";
        try (Connection c = SessionFactory.getInstance().getConnection();
             PreparedStatement ps = c.prepareStatement(sql)) {
            ps.setLong(1, id);
    ...
    public void create(DossierMedical d) {
        String sql = """
            INSERT INTO DossiersMedical(historique, pat_id, medcine_id, dateCreation, creePar)
            VALUES(?,?,?,?,?)
            """;
        try (Connection c = SessionFactory.getInstance().getConnection();
    ...
    public void deleteById(Long id) {
        String sql = "DELETE FROM DossiersMedical WHERE id = ?";
        try (Connection c = SessionFactory.getInstance().getConnection();
             PreparedStatement ps = c.prepareStatement(sql)) {
            ps.setLong(1, id);
            ps.executeUpdate();
    ...
    public Optional<DossierMedical> findByPatientId(Long patientId) {
        String sql = "SELECT * FROM DossiersMedical WHERE pat_id = ?";
        try (Connection c = SessionFactory.getInstance().getConnection();
             PreparedStatement ps = c.prepareStatement(sql)) {
    ...
    public List<DossierMedical> findByMedecinId(Long medecinId) {
        String sql = "SELECT * FROM DossiersMedical WHERE medcine_id = ?";
        List<DossierMedical> out = new ArrayList<>();
        try (Connection c = SessionFactory.getInstance().getConnection();
             PreparedStatement ps = c.prepareStatement(sql)) {
    ...
    @Override
    public long count() {
        String sql = "SELECT COUNT(*) FROM DossiersMedical";
        try (Connection c = SessionFactory.getInstance().getConnection();
             PreparedStatement ps = c.prepareStatement(sql);
             ResultSet rs = ps.executeQuery()) {
            rs.next();
