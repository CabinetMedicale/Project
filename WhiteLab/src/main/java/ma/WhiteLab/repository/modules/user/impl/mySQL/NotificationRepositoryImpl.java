...
import ma.WhiteLab.repository.modules.user.api.NotificationRepository;
...
public class NotificationRepositoryImpl implements NotificationRepository {
    ...
    public void create(Notification n) {
        String sql = """
            INSERT INTO Notifications(titre, message, date, time, type, priorite, dateCreation, creePar)
            VALUES(?,?,?,?,?,?,?,?)
            """;
        try (Connection c = SessionFactory.getInstance().getConnection();
    ...
    public Optional<Notification> findByTitre(String titre) {
        String sql = "SELECT * FROM Notifications WHERE titre = ?";
        try (Connection c = SessionFactory.getInstance().getConnection();
             PreparedStatement ps = c.prepareStatement(sql)) {
    ...
    public List<Notification> findByType(String type) {
        String sql = "SELECT * FROM Notifications WHERE type = ? ORDER BY date DESC, time DESC";
        List<Notification> out = new ArrayList<>();
        try (Connection c = SessionFactory.getInstance().getConnection();
             PreparedStatement ps = c.prepareStatement(sql)) {
            ps.setString(1, type);
    ...
    public List<Notification> findByPriorite(String priorite) {
        String sql = "SELECT * FROM Notifications WHERE priorite = ? ORDER BY date DESC, time DESC";
        List<Notification> out = new ArrayList<>();
        try (Connection c = SessionFactory.getInstance().getConnection();
             PreparedStatement ps = c.prepareStatement(sql)) {
